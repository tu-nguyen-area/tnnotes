...
export default function EditForm({ note }: { note: Note }) {
  ...
  const editNoteId = editNote.bind(null, note.note_id );
    ...
    <form action={formAction}>
      <div className="my-6">
        <label className="text-xl bg-gradient-to-r from-indigo-500 to-pink-500 bg-clip-text font-bold text-transparent">Title</label>
          <input name="title" defaultValue={note.title} placeholder="Title..." className="rounded-lg w-full border-2 p-2" required></input>
      </div>
      <div>
        <input name="author_id" defaultValue={note.author_id} type="hidden" className=""></input>
      </div>
      <div className="my-6">
        ...
        <textarea name="content" defaultValue={note.content} placeholder="Content..." className="rounded-lg w-full h-64 md:h-96 border-2 p-2" required></textarea>
      </div>
      <div className="grid gap-6 md:flex md:justify-around my-6 md:my-12">
        <Link href={`/home/${note.note_id}`}>
