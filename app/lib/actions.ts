...
export async function searchNote(id: string, search: string) {
  const lower = search.toLowerCase();
  const upper = search.toUpperCase();
  const first = search.charAt(0).toUpperCase().concat(lower.substr(1));
  ...
  try {
    ...
    console.error('Database Error:', error);
    throw new Error('Failed to search note data.');
  }
}
